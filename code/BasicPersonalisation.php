...
//		$rules = $this->generateRulesList();
    ...
    public function generateRulesList()
    {
    ...
    **/
    public function getRulesList()
    {
        ...
        if ($rules = $this->Rules()->filter("ParentID", $this->ID)->sort('Priority ASC')) {
            foreach ($rules as $rule) {
    ...
    public function hasDefault()
    {
        foreach ($this->Rules() as $rule) {
            ...
            if (($decodedRule[0]) && ($decodedRule[0]->operator == BasicPersonalisationCondition::$op__always)) {
                return true;
            }
        }
        return false;
