...
class BasicPersonalisationRule extends DataObject
{
     ...
     * @return void
     */
    public function setCondition($cond, $write = true)
    {
        if (!is_array($cond)) {
            throw new Exception("BasicPersonalisationRule::setCondition expects an array of conditions");
        }
    ...
    public function variationOnMatch($context)
    {
        $conds = $this->getCondition();
...
{
    public static $op__always = "always";            // requires no parameters
    public static $op__equals = "eq";                // requires 2 parameters
    public static $op__notequals = "ne";            // requires 2 parameters
    public static $op__contains = "contains";        // requires 2 string parameters
    ...
    public function __construct($operator = null, $param1 = null, $param2 = null)
    {
    ...
    public function getRequiredProperties()
    {
        $result = array();
        if ($this->param1) {
            $this->param1->addPropertyRequest($result);
        }
        if ($this->param2) {
            $this->param2->addPropertyRequest($result);
        }
        return $result;
    }
     ...
     * @return void
     */
    public function IsTrue($context)
    {
        if ($this->operator == self::$op__always) {
            return true;
        }

        // All other operators require both parameters, so evaluate them now.
        if (!$this->param1 || !$this->param2) {
            throw new Exception("Trying to evaluate a rule that requires 2 parameters, but parameters are missing");
        }
...
{
    public static $op__literal = "L";
    public static $op__property = "P";

    public $kind;
    public $value;

    public static function make_const($const)
    {
    ...
    public static function make_property($property)
    {
        $v = new BasicPersonalisationValue();
    ...
    public function getValue($context)
    {
        switch ($this->kind) {
            ...
            case self::$op__property:
                ...
                if (!isset($props[$this->value]) || !is_array($props[$this->value]) || count($props[$this->value]) == 0) {
                    return null;
                }
     ...
     * @return void
     */
    public function addPropertyRequest(&$array)
    {
        if ($this->kind != self::$op__property) {
            return;
        }
...
class RuleEditField extends FormField
{
    ...
    public function __construct($name, $title = null, $value = null)
    {
        ...
        if ($value !== null) {
            $this->setValue(BasicPersonalisationRule::json_decode_typed($value));
        }
...
class PersonalisationRuleHelper extends Controller
{
    ...
    public function on_after_sort()
    {
        if (!isset($_POST['ruleIDs']) || $_POST['ruleIDs'] == '') {
            return 'bad';
        }
