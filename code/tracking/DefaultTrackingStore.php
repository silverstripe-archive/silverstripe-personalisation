...
class DefaultTrackingStore implements TrackingStore
{
    ...
    public function getProperties(array $identities, $properties)
    {
    ...
    public function setProperties(array $identities, $properties)
    {
        if (!is_array($identities)) {
            return;
        } // do nothing if the identity is not provided
        if (!is_array($properties)) {
            return;
     ...
     * @var array
     */
    public static $property_ids = array();
     ...
     * @return void
     */
    public function getPropertyID($propertyName, $createOnDemand = false)
    {
        if (!isset(self::$property_ids[$propertyName])) {
            ...
            if (!$propID) {
                if ($createOnDemand) {
                    ...
                    return null;
                }  // We're not creating it, and it's not defined, so return nothing.
            }
    ...
    protected function executeFunction($fn, $inputData)
    {
        ...
        if (! in_array($name, self::$supported_functions)) {
            throw new Exception("executeFunction: $name is not a supported function");
        }
    ...
    protected function executeFnGetEvents($params, $data)
    {
        ...
        if (!isset($params["property"])) {
            throw new Exception("executeFnGetEvents: property must be supplied");
        }
        ...
        if (!is_array($props)) {
            throw new Exception("executeFnGetEvents: properties must be an array or single string");
        }
    ...
    protected function executeFnCountByTime($params, $data)
    {
        ...
        if (!isset(self::$periods[$period])) {
            throw new Exception("Invalid periond $period");
        }
